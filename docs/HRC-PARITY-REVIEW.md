...
| `dpts_6` Member Invite Flow | no credit limit on invite, no expiry column, no welcome-card preview | Credit limit **and** it is really applied on accept; an Expiration column; the welcome-card preview | Two rounds of defect. (1) Invitations never expired server-side — a year-old invite still allocated its credit line, and the Expiration column was `created_at + a client constant`, a date the browser invented. (2) The invite was **write-only**: `club_invite` stored a row and notified nobody, the one caller of `club_invitations_list` was in the dead `sections/` tree, and `club_request_review` — fully implemented, expiry check and credit allocation and all — was never called with `accept` by anything. The invited player never learned they were invited and could not have accepted if they had. |
...
| `dpts_6` Member Invite Flow | Email-or-wallet, **assign initial credit limit**, welcome-card preview, pending invitations with Status / Sent / Expiration | ✅ **DONE**. Every control existed, including the welcome-card preview this doc called missing. The defects were underneath: invitations never expired server-side, and the invite was write-only — no notification, no live caller of the list RPC, and nothing that ever called `club_request_review` with `accept`. The card is now delivered to the invitee with working Accept/Decline. |
