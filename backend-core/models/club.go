...
type Club struct {
	...
	// Timezone is an IANA zone (e.g. "America/New_York"). It is what
	// poker_club_schedule.time_of_day is resolved in — that column stores a bare
	// 'HH:MM' commented "operator-local" and previously had no zone to resolve
	// against, so a club night had no well-defined moment.
	Timezone string `json:"timezone" db:"timezone" label:"Timezone" help:"IANA zone. Club-night schedules fire in it."`
	// PrimaryLanguage labels the club in the public browser. It is NOT an
	// interface language — there is no i18n system in this app — so it must never
	// be presented as one.
	PrimaryLanguage string `json:"primary_language" db:"primary_language" label:"Primary Language" help:"Shown on the club's public listing. Does not translate the interface."`
	// TwoFARequired refuses club operator actions from an account with no TOTP
	// enrolled. Enforced once, in requireClubConfigurer.
	TwoFARequired bool      `json:"twofa_required" db:"twofa_required" label:"Require 2FA for operators"`
...
type Owner struct {
	...
	Role         string `json:"role" db:"role" enum:"owner,manager,agent,moderator" label:"Role"` // owner | manager | agent | moderator
	...
	// Permissions is a comma-separated slug list narrowing what this seat may do
	// (manage_members / manage_money / manage_tables / manage_settings).
	//
	// EMPTY MEANS FULL ACCESS — every seat created before the permission grid has
	// an empty value, and they must keep the authority they already had. A seat is
	// only constrained once an owner assigns permissions to it. Owners are never
	// constrained at all. See rpc/clubperm.go.
	Permissions string    `json:"permissions" db:"permissions" label:"Permissions" help:"Leave empty for full access."`
	CreatedAt   time.Time `json:"created_at" db:"created_at" server:"true"`
	UpdatedAt   time.Time `json:"updated_at" db:"updated_at" server:"true"`
}
